 ...
 */
package com.rave.yunwang.widget.smarttablayout;
...
public class SmartTabLayout extends HorizontalScrollView {
  ...
  /**
   * Sets the custom tab views and associated view pager.
   * @param tabViews the custom tab views
   * @param viewPager associated view pager.
   */
  public void setCustomTabViews(List<View> tabViews, ViewPager viewPager) {
    tabStrip.removeAllViews();
    this.viewPager = viewPager;

    if(tabViews != null && viewPager != null && viewPager.getAdapter()!= null && viewPager.getAdapter().getCount() == tabViews.size()){
      viewPager.addOnPageChangeListener(new InternalViewPagerListener());
      populateTabStrip(tabViews);
    }
   ...
   * {@link #setCustomTabView(int, int)}.
   */
  private void populateTabStrip(List<View> views){
    for (int i = 0; i < views.size(); i++) {

      final View tabView = views.get(i);
